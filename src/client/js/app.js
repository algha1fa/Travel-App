...
function performAction(e) {
    ...
    getlocation(baseURL1, zipcode, cityname, countrycode, username)
        .then(function(data) {
            ...
            var Difference_In_Days = parseInt((date2.getTime() - date1.getTime()) / (1000 * 3600 * 24));
            //alert("DIff = " + Difference_In_Days)

            var d3 = document.getElementById('edate').value;
            var date3 = new Date(d3 + "Z");
            var tripdays = parseInt((date3.getTime() - date2.getTime()) / (1000 * 3600 * 24));
            ...
            if (Difference_In_Days <= 7) {
                ...
                getweather(baseURL2, url2apiKey, lat, lng).then(
                    function(data) {
                        //console.log(data)
                        ...
                        line1.innerHTML = "My trip to " + placename + "," + cityname + "<br> Departing : " + d2 + " <br> Weather forecast is : <br>"
                        ...
                        line4.innerHTML = "<b>Total No of days for this trip is:</b> " + tripdays + " days"
                        ...
                        getpic(baseURL3, url3apiKey, search).then(
                            function(data) {
                ...
                getforecast(baseURL4, url2apiKey, lat, lng, sdate, edate).then(
                    function(data) {
                        ...
                        const line1 = document.createElement("h2");
                        line1.innerHTML = "My trip to " + placename + "," + cityname + "<br> Departing : " + d2 + " </h2><br> Trip is " + Difference_In_Days + " days Away .<br>Typical weather for then is <br> "
...
const getpic = async(baseURL, apikey, search) => {
    let str = baseURL + apikey + "&q=" + encodeURIComponent(search) + "&category=place&image_type=photo"
        //    alert("in getpic")
        //  alert(str)
    console.log(str)
...
const getforecast = async(baseURL, url2apiKey, lat, lng, startdate, enddate) => {
    //alert("in forecast")
    let str = baseURL + url2apiKey + "&lat=" + lat + "&lon=" + lng + "&start_date=" + startdate + "&end_date=" + enddate
    alert(str)
    console.log(str)
    const res = await fetch(str, {
...
const getlocation = async(baseURL, zipcode, cityname, countrycode, uname) => {
    ...
    let str = baseURL + zipcode + "&placename=" + cityname + "&country=" + countrycode + "&username=" + uname
        // alert(str)
    const res = await fetch(str)
    try {
